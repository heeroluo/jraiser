...
var idManager = (function() {
	...
	var autoId = 0; // 自增id
	var specialObjData = {}; // 存放非HTML节点的数据
	...
	// Chrome 57+ 在某些情况下（未查明）会回收节点的expando特性；
	// 生成一个闭包并记录到此对象中以阻止回收。
	var keepNodeExpando = {};
...
var DataSpace = base.createClass(function(options) {
	...
	// 寻找节点对应的数据空间
	_findData: function(node) {
		return this._space[idManager.get(node, true)];
	},
	...
	// 清空节点数据
	clear: function(node) {
		delete this._space[idManager.get(node, true)];
	},
	...
	clone: function(targetNode, sourceNode) {
		...
		if (sourceData) {
			var targetId = idManager.get(targetNode);
			var targetData = this._space[targetId] = this._space[targetId] || { };
 ...
 */
var createDataSpace = exports.createDataSpace = function(options) {
 ...
 */
exports.clearAll = function(node) {
	dataSpaceManger.clear(node);
	idManager.remove(node);
};
 ...
 */
exports.cloneAll = function(targetNode, sourceNode) {
	dataSpaceManger.clone(targetNode, sourceNode);
};
 ...
 */
exports.removeUniqueId = function(node) {
	idManager.remove(node);
};
...
function getData(node, key) {
	return userDataSpace.get(node, key);
}

// 设置节点数据
function setData(node, key, value) {
	userDataSpace.set(node, key, value);
}
