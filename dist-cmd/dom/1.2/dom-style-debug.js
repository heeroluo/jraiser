...
// 匹配相对数字，如 +=10
var rRelNumber = exports.rRelNumber = /^([+-])=(\d+(?:\.\d+)?)$/;
...
if (!('opacity' in testNode.style)) {
	var reOpacity = /opacity\s*=\s*([^)]*)/i;
	var reAlpha = /alpha\([^)]*\)/i;

	// 获取/设置透明度，Only For IE<9
	cssHooks.opacity = {
		get: function(node) {
			return reOpacity.test(node.currentStyle.filter || '') ?
		...
		set: function(node, value) {
			...
			style.zoom = 1;
			if ((isNaN(value) || value >= 1) && filter.replace(reAlpha, '').trim() === '') {
				style.removeAttribute('filter');
			} else {
				var opacity = 'alpha(opacity=' + Math.min(1, value) * 100 + ')';
				style.filter = reAlpha.test(filter) ?
					filter.replace(reAlpha, opacity) : filter + ' ' + opacity;
			}
		}
	};
}
...
function fixStyleName(name) {
	...
	var origName = name.replace(rDash, function(match, $1) {
 ...
 * @return {String} 样式值。
 */
var getStyle = exports.getStyle = function(node, name) {
 ...
 */
var setStyle = exports.setStyle = function(node, name, val) {
	if (!name || !domBase.isHTMLElement(node)) { return; }

	// 计算相对值，例如 +=5
	if (rRelNumber.test(val)) {
		val = (parseFloat(getStyle(node, name), 10) || 0) +
			parseFloat(RegExp.$1 + RegExp.$2, 10);
	}
...
function hasClass(node, className) {
	if (!className) { throw new Error('Classname not specified'); }
...
// 如果元素的display样式属性设成了none，
// 要让其可见就得把该样式属性的值设成元素对应的display值（inline、block或table等），
// 此对象主要获取、记录元素对应的display值。
...
exports.shortcuts = {
	 ...
	 * @method width
	 * @for NodeList
	 * @param {Number|String} val 宽度值（支持相对值，如+=N、-=N）。
	 * @return {NodeList} 当前节点集合。
	 */
	width: function(val) {
	 ...
	 * @method height
	 * @for NodeList
	 * @param {Number|String} val 高度值（支持相对值，如+=N、-=N）。
	 * @return {NodeList} 当前节点集合。
	 */
	height: function(val) {
