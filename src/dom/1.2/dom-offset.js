...
function getPosition(node) {
	...
	if (domStyle.getStyle(node, 'position') === 'fixed') {
		...
		parentOffset.top += parseFloat(
			domStyle.getStyle(offsetParent, 'borderTopWidth')
		) || 0;
		parentOffset.left += parseFloat(
			domStyle.getStyle(offsetParent, 'borderLeftWidth')
		) || 0;
	}

	return {
		top: offset.top - parentOffset.top - (
			parseFloat(domStyle.getStyle(node, 'marginTop')) || 0
		),
		left: offset.left - parentOffset.left - (
			parseFloat(domStyle.getStyle(node, 'marginLeft')) || 0
		)
	};
}
