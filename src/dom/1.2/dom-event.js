...
var EventArg = require('./dom-event-arg');
...
// 存放各种事件类型对应的事件类别（在文件末尾处赋值）
var eventTypes = { };

// 事件参数标准化 (主要代码来自jQuery)
var eventArgNormalizer = {
	...
	fix: function(e) {
		...
		var originalEvent = e;
		var hook = this[eventTypes[e.type] + 'Hook'];
		var props = hook && hook.props ? this.props.concat(hook.props) : this.props;

		e = new EventArg(e);
		props.forEach(function(item) {
			if (item in originalEvent) {
				e[item] = originalEvent[item];
			}
		});

		// Fix target property, if necessary (#1925, IE 6/7/8 & Safari2)
		if (!e.target) { e.target = originalEvent.srcElement || document; }
...
// 旧版本的Chrome、Safari、Firefox、Opera不支持mouseenter/mouseleave，需要模拟
// 原生mousenter/mouseleave不冒泡，无法进行事件代理，所以统一采用mouseover/mouseleave处理
base.each({
...
function dispatch(e) {
	...
	var listeners = listenerManager.get(node, bindType);
	...
	if (listeners) {
		...
		listeners.forEach(function(listener) {
			...
			var result = listener.handle ?
				listener.handle.call(thisObj, listener, e) :
				listener.handler.call(thisObj, e);
...
listenerManager = (function() {
	...
	function bindDispatch(node, bindType, listenerSpace) {
		...
		if (isOldIE) {
			...
			myDispatch = function() { return dispatch.apply(node, arguments); };
...
// 支持事件操作时才执行
function doIfSupportEvent(fn) {
	return function(node) {
		if (domBase.isWindow(node) || domBase.isHTMLElement(node, true)) {
...
// 在指定节点上注册事件监听
var on = doIfSupportEvent(function(node, types, handler, options) {
...
// 在指定节点上移除事件监听
var off = doIfSupportEvent(function(node, types, handler) {
	if (types) {
		types.forEach(function(type) {
			type = type.split('.');
			var hook = eventHooks[type[0]];
...
// 模拟事件触发
var trigger = doIfSupportEvent(function(node, type, options) {
	if (!type) { return; }
	...
	var bubbles = options.bubbles != null ? options.bubbles : bubbleEvents[type];
...
(
	'blur focus focusin focusout load resize scroll unload contextmenu click dblclick ' +
	'mousedown mouseup mousemove mouseover mouseout mouseenter mouseleave ' +
	'touchstart touchmove touchend change select submit keydown keypress keyup'
