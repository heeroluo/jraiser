...
var domIns = require('./dom-insertion');
...
var domAnimation = require('./dom-animation');
...
function querySelector(selector, context) {
	if (context != null &&
		context.length &&
		!domBase.isNode(context) &&
		!domBase.isWindow(context)
	) {
		// 有多个context，逐个执行selector匹配
		var i = -1, len = context.length, result = [];
		while (++i < len) {
			Sizzle(selector, context[i], result); // eslint-disable-line new-cap
		}
		if (len > 1) { Sizzle.uniqueSort(result); }
		return result;
	} else {
		return Sizzle(selector, context); // eslint-disable-line new-cap
	}
}
