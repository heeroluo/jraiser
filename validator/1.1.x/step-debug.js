 ...
 *   @param {Boolean} [options.oneByOne=true] 是否对字段值逐个验证
...
return widget.create({
	 ...
	 */
	exec: function(elements, values, helpers, ignoreRemote, sourceEvent) {
		var t = this;
		...
		if (ruleNames) {
			for (var i = ruleNames.length - 1; i >= 0; i--) {
				if (!helpers[ ruleNames[i] ]) {
					throw new Error('Rule "' + ruleNames[i] + '" does not exist');
				}
			}
		}
		...
		if (totalFields !== 1 || options.oneByOne === false || options.remoteURL) {
			...
			if (result && t._rule) {
				if (options.remoteURL) {
					var onRemoteSuccess = function(res) {
						t._remoteCache = res;
						t._message = t._rule.call(window, res);
						if (t._message) {
							return t._error(elements.slice(), true, sourceEvent);
						} else {
							t._correct(elements.slice(), true, sourceEvent);
						}
					};
		...
		if (result) {
			t._correct(elements.slice(), false, sourceEvent);
		} else {
			return t._error(errElements, false, sourceEvent);
		}
	},
	 ...
	 * @return {Object} 事件参数
	 */
	_beforeSend: function(elements, sourceEvent) {
	 ...
	 * @return {Object} 事件参数
	 */
	_error: function(elements, isRemote, sourceEvent) {
		var e = this._makeEventArg({
			sourceEvent: sourceEvent,
			elements: elements,
			isRemote: !!isRemote,
	 ...
	 * @return {Object} 事件参数
	 */
	_correct: function(elements, isRemote, sourceEvent) {
		var e = this._makeEventArg({
			sourceEvent: sourceEvent,
			elements: elements,
			isRemote: !!isRemote
